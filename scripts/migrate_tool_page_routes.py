"""Move tool page route registration out of app.py.

This is a conservative migration. It removes only the route decorators from
app.py and registers wrappers in craftworld_tools.routes.pages_tools_legacy.
The legacy function bodies remain in app.py for now.

Run from a local checkout:

    python scripts/migrate_tool_page_routes.py
    python -m py_compile app.py
    python scripts/audit_routes.py

It writes `app.py.tool_pages.bak` before changing the file.
"""
...
APP = ROOT / "app.py"
BACKUP = ROOT / "app.py.tool_pages.bak"

IMPORT_LINE = "from craftworld_tools.routes.pages_tools_legacy import register_tool_page_legacy_routes\n"
REGISTER_LINE = "\n# Extracted tool page route registration\nregister_tool_page_legacy_routes(app)\n"

TARGET_ROUTES = {
    "/calculate": "calculate",
    "/upgrade-calculate": "upgrade_calculate",
    "/factory-converter": "factory_converter",
    "/flex": "flex_planner",
    "/snipe": "snipe",
}
...
def add_registration(text: str) -> str:
    if "register_tool_page_legacy_routes(app)" in text:
        return text
    anchors = [
...
def main() -> None:
    ...
    print("Moved tool page route registration out of app.py and wrote app.py.tool_pages.bak")
