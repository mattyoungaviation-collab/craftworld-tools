"""Move lookup page route registration out of app.py.

This is a conservative migration. It removes only the route decorators from
app.py and registers wrappers in craftworld_tools.routes.pages_lookup_legacy.
The legacy function bodies remain in app.py for now.

Run from a local checkout:

    python scripts/migrate_lookup_page_routes.py
    python -m py_compile app.py
    python scripts/audit_routes.py

It writes `app.py.lookup_pages.bak` before changing the file.
"""
...
APP = ROOT / "app.py"
BACKUP = ROOT / "app.py.lookup_pages.bak"

IMPORT_LINE = "from craftworld_tools.routes.pages_lookup_legacy import register_lookup_page_legacy_routes\n"
REGISTER_LINE = "\n# Extracted lookup page route registration\nregister_lookup_page_legacy_routes(app)\n"

TARGET_ROUTES = {
    "/inventory": "inventory_view",
    "/mastery": "mastery_view",
    "/resource/<token>": "resource_view",
    "/player/<uid>": "player_view",
}
...
def add_registration(text: str) -> str:
    if "register_lookup_page_legacy_routes(app)" in text:
        return text
    anchors = [
...
def main() -> None:
    ...
    print("Moved lookup page route registration out of app.py and wrote app.py.lookup_pages.bak")
