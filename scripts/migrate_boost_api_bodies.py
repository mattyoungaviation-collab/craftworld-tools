"""Move boost API function bodies out of app.py.

This migrates these legacy handlers into craftworld_tools/routes/api_boosts.py:
...
Run from a local checkout:
    ...
    python scripts/migrate_boost_api_bodies.py
    python -m py_compile app.py craftworld_tools/routes/api_boosts.py craftworld_tools/routes/api_boosts_legacy.py
    python scripts/audit_routes.py

It writes `app.py.boost_api_bodies.bak` before changing app.py.
"""
...
APP = ROOT / "app.py"
DEST = ROOT / "craftworld_tools" / "routes" / "api_boosts.py"
WRAPPER = ROOT / "craftworld_tools" / "routes" / "api_boosts_legacy.py"
BACKUP = ROOT / "app.py.boost_api_bodies.bak"

FUNCTIONS = ["api_boosts_mastery", "api_boosts_sync"]

DEST_HEADER = '''"""Boost API handlers.
...
WRAPPER_CONTENT = '''"""Boost API route wrappers."""
...
def _find_source_text(app_text: str) -> tuple[dict[str, str], str, str]:
    ...
    raise RuntimeError(
        "Could not find all boost API function bodies in app.py or backups. Missing from app.py: "
...
def main() -> None:
    ...
    print(f"Moved boost API bodies using {source_name} and wrote app.py.boost_api_bodies.bak")
