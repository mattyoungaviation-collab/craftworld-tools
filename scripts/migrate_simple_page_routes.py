"""Move simple page route registration out of app.py.

This is a conservative migration. It removes only the route decorators from
app.py and registers wrappers in craftworld_tools.routes.pages_simple_legacy.
The legacy function bodies remain in app.py for now.

Run from a local checkout:

    python scripts/migrate_simple_page_routes.py
    python -m py_compile app.py
    python scripts/audit_routes.py

It writes `app.py.simple_pages.bak` before changing the file.
"""
...
APP = ROOT / "app.py"
BACKUP = ROOT / "app.py.simple_pages.bak"

IMPORT_LINE = "from craftworld_tools.routes.pages_simple_legacy import register_simple_page_legacy_routes\n"
...
def add_registration(text: str) -> str:
    if "register_simple_page_legacy_routes(app)" in text:
        return text
    anchors = [
...
def main() -> None:
    ...
    print("Moved simple page route registration out of app.py and wrote app.py.simple_pages.bak")
