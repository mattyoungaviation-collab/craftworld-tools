...
# Ronin token addresses (we mainly need COIN to USD)
...
def fetch_exchange_prices_buy_sell() -> Dict[str, Dict[str, float]]:
    ...
    for item in root.get("prices", []):
        ...
        try:
            ...
            amt_f = 0.0
        per_symbol.setdefault(sym, {})[rec if rec else "UNKNOWN"] = amt_f

    per_symbol.setdefault(base_symbol, {})
...
def _fetch_exact_input_quote(input_symbol: str, output_symbol: str, input_amount: float) -> Optional[dict]:
    variables = {
        "input": {
...
def fetch_buy_sell_for_profitability(symbols: List[str], sample_amount: float = 2) -> Dict[str, Dict[str, float]]:
    base = fetch_exchange_prices_buy_sell()
    per_symbol: Dict[str, Dict[str, float]] = {sym_u.upper(): dict(rec_map) for sym_u, rec_map in base.items()}
...
def fetch_exchange_prices_coin() -> Dict[str, float]:
    ...
    for sym_u, rec_map in per_symbol.items():
        ...
        if selected is not None:
            prices_coin[sym_u.upper()] = selected
...
def fetch_live_prices_in_coin() -> Dict[str, float]:
    ...
    if not fish_price:
        ...
        if quote_fish_coin and quote_fish_coin.get("output") and quote_fish_coin.get("input"):
            try:
    ...
    if not fish_price:
        ...
        if quote_coin_fish and quote_coin_fish.get("output") and quote_coin_fish.get("input"):
            try:
