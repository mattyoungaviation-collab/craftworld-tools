...
from craftworld_api import fetch_craftworld, fetch_masterpiece_details, fetch_masterpieces
...
def cached_masterpiece_details(masterpiece_id: int, ttl_seconds: float = 120.0) -> dict[str, Any]:
    """Fetch one Masterpiece detail payload with a short cache window."""
    mid = int(masterpiece_id)
    return _cache.get_or_set(("masterpiece_details", mid), ttl_seconds, lambda: fetch_masterpiece_details(mid))
