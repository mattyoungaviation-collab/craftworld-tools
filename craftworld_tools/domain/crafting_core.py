...
from craftworld_tools.domain.factories import FACTORIES_FROM_CSV, MASTERY_BONUSES, WORKSHOP_MODIFIERS
...
def plan_craft(
    ...
    def expand(symbol: str, qty: float) -> None:
        ...
        if s in BASE_SYMBOLS:
            external_needs[s] = external_needs.get(s, 0.0) + qty
            if s not in available_base_set and s not in prices and s not in base_cost_model:
                missing_prices.add(s)
            return
