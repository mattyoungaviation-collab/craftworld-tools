...
def profit_per_hour(factory: MyFactory, prices: Dict[str, float], speed_factor: float = 1.0, workers: int = 0) -> float:
    workers_clamped = max(0, min(workers, 4))
    multiplier = max(speed_factor, 0.01) * (1.0 + 0.5 * workers_clamped)
    eff_duration_hours = factory.duration_hours / multiplier if multiplier > 0 else factory.duration_hours
...
def load_factories_from_csv(path: str) -> Dict[str, Dict[int, dict]]:
    ...
    try:
        with open(path, newline="", encoding="utf-8-sig") as f:
            ...
            for row in r:
                ...
                try:
                    ...
                    continue
                try:
                    duration_min = float(str(row.get("duration_min")))
                except Exception:
                    duration_min = 0.0
                out_token_raw = _normalize_token(row.get("output_token") or token)
                out_token = out_token_raw if out_token_raw else token
                try:
                    output_amount = float(str(row.get("output_amount")).replace(",", ""))
                except Exception:
                    output_amount = 0.0
                inputs: Dict[str, float] = {}
                for idx in (1, 2):
                    t_raw = _normalize_token(row.get(f"input_token_{idx}") or "")
                    if not t_raw:
                        continue
                    q_raw = row.get(f"input_amount_{idx}")
                    if q_raw in (None, ""):
                        continue
                    try:
                        ...
                        qty = 0.0
                    inputs[t_raw] = inputs.get(t_raw, 0.0) + qty
        ...
        print("Error loading factories from CSV:", e)
        factories = {}
    return factories
...
FACTORY_DISPLAY_ORDER: list[str] = [tok for tok in _FACTORY_DISPLAY_ORDER_BASE if tok in FACTORIES_FROM_CSV]
for tok in sorted(FACTORIES_FROM_CSV.keys()):
    if tok not in FACTORY_DISPLAY_ORDER:
        FACTORY_DISPLAY_ORDER.append(tok)

FACTORY_DISPLAY_INDEX: dict[str, int] = {tok: idx for idx, tok in enumerate(FACTORY_DISPLAY_ORDER)}
